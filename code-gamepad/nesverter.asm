...
; command line:  ./jalv2osx -long-start -s ../lib /Users/picxpert/Desktop/nesverter.jal -loader18 -no-fuse
...
v__btemp53                     EQU 0x0027  ; _btemp53-->_bitbucket:0
v__floop10                     EQU 0x0029  ; usb_handle_isr:_floop10
v___end_point_2                EQU 0x002b  ; _usb_handle_transaction:end_point
...
v_descriptor_type              EQU 0x0034  ; _usb_handle_standard_request:descriptor_type
v_descriptor_num               EQU 0x0035  ; _usb_handle_standard_request:descriptor_num
v___dts_bit_5                  EQU 0x0204  ; _usb_handle_standard_request:dts_bit-->usb_bd0in_stat:6
v___end_point_1                EQU 0x0034  ; usb_prime_epx_out:end_point
v___size_1                     EQU 0x0035  ; usb_prime_epx_out:size
v____temp_27                   EQU 0x0036  ; usb_prime_epx_out:_temp
v___idx_1                      EQU 0x0037  ; usb_prime_epx_out:idx
v_new_value                    EQU 0x0034  ; usb_prime_ep0_out:new_value
...
v___byte_cnt_2                 EQU 0x0035  ; usb_send_data:byte_cnt
v___is_new_sequence_1          EQU 0x0036  ; usb_send_data:is_new_sequence-->_bitbucket15:0
v____bitbucket_15              EQU 0x0036  ; usb_send_data:_bitbucket
...
v___data_3                     EQU 0x0036  ; usb_send_one_byte:data
...
v_byte_cnt                     EQU 0x0037  ; usb_send_data_chunk:byte_cnt
v_dts_bit                      EQU 0x0204  ; usb_send_data_chunk:dts_bit-->usb_bd0in_stat:6
v___dst_1                      EQU 0x0037  ; _usb_copy_array_to_ram:dst
v___data_1                     EQU 0x0039  ; _usb_copy_array_to_ram:data
v___num_bytes_1                EQU 0x003c  ; _usb_copy_array_to_ram:num_bytes
v_index                        EQU 0x003d  ; _usb_copy_array_to_ram:index
v_dst_ptr                      EQU 0x0037  ; _usb_copy_array_to_ram:dst_ptr-->dst1
v__floop9                      EQU 0x003e  ; _usb_copy_array_to_ram:_floop9
...
;  194 			if is_new_sequence then
                               movlb    0
                               btfss    v____bitbucket_15, 0,v__banked ; is_new_sequence1
                               goto     l__l188
;  196 				dts_bit = low
                               movlb    2
...
;  317 	end_point = USTAT >> 3;
                               ...
                               movwf    v___end_point_2,v__banked
                               bcf      v__status, v__c,v__access
                               rrcf     v___end_point_2,f,v__banked
                               bcf      v__status, v__c,v__access
                               rrcf     v___end_point_2,f,v__banked
...
;  356 	if ( end_point == 0 ) then
                               movf     v___end_point_2,w,v__banked
                               btfss    v__status, v__z,v__access
                               goto     l__l330
;  365 		case pid of
;  366 			USB_PID_SETUP:
...
;  638 		if USTAT_DIR then
                               btfsc    v_ustat, 2,v__access ; ustat_dir
;  664 		else
                               return   
l__l411
...
l__l482
                               return   
l__l485
; /Users/picxpert/Desktop/nesverter.jal
;  182 var byte hid_report_in[8] = { 0,0,0,0,0,0,0,0 }
...
;  184 var byte xAxis = 0
                               clrf     v_xaxis,v__banked
;  185 var byte xAxisNew = 128
                               movlw    128
                               movwf    v_xaxisnew,v__banked
;  186 var byte yAxis = 0
                               clrf     v_yaxis,v__banked
;  187 var byte yAxisNew = 128
                               movlw    128
                               movwf    v_yaxisnew,v__banked
;  188 var byte buttons = 255
                               movlw    255
                               movwf    v_buttons,v__banked
;  189 var byte buttonsNew = 0
                               clrf     v_buttonsnew,v__banked
;  191 var byte changedFlag = 0
                               clrf     v_changedflag,v__banked
;  203 enable_digital_io()
; ../lib/18f14k50.jal
...
;  236         xAxisNew = 128
                               movlw    128
                               movwf    v_xaxisnew,v__banked
;  237         if(leftButton == low) then
                               btfsc    v_portc, 4,v__access ; pin_c4
                               goto     l__l503
;  238             xAxisNew = xAxisNew - 128
                               movlw    128
                               addwf    v_xaxisnew,f,v__banked
;  239         end if
l__l503
;  241         if(rightButton == low) then
                               btfsc    v_portc, 5,v__access ; pin_c5
                               goto     l__l505
;  242             xAxisNew = xAxisNew + 127
                               movlw    127
                               addwf    v_xaxisnew,f,v__banked
;  243         end if
l__l505
;  245         buttonsNew = 0
                               clrf     v_buttonsnew,v__banked
;  247         if(aButton == low) then
                               btfsc    v_portb, 7,v__access ; pin_b7
                               goto     l__l507
;  248             buttonsNew = buttonsNew + 1
                               incf     v_buttonsnew,f,v__banked
;  249         end if
l__l507
;  251         if(bButton == low) then
                               btfsc    v_portc, 1,v__access ; pin_c1
                               goto     l__l509
;  252             buttonsNew = buttonsNew + 2
                               movlw    2
                               addwf    v_buttonsnew,f,v__banked
;  253         end if
l__l509
;  255         if(selectButton == low) then
                               btfsc    v_portc, 2,v__access ; pin_c2
                               goto     l__l511
;  256             buttonsNew = buttonsNew + 4
                               movlw    4
                               addwf    v_buttonsnew,f,v__banked
;  257         end if
l__l511
;  259         if(startButton == low) then
                               btfsc    v_portb, 5,v__access ; pin_b5
                               goto     l__l513
;  260             buttonsNew = buttonsNew + 8
                               movlw    8
                               addwf    v_buttonsnew,f,v__banked
;  261         end if
l__l513
;  263         if(xAxis != xAxisNew) then
...
;  271         end if
l__l517
;  273         if(buttons != buttonsNew) then
...
;  274             changedFlag = 1
                               movlw    1
                               movwf    v_changedflag,v__banked
;  275             buttons = buttonsNew
                               movf     v_buttonsnew,w,v__banked
                               movwf    v_buttons,v__banked
;  276         end if
l__l519
;  279 		if ( changedFlag == 1 ) then
                               movlw    1
...
;  281 			changedFlag = 0
                               clrf     v_changedflag,v__banked
;  283             hid_report_in[0] = xAxis
                               movf     v_xaxis,w,v__banked
                               movwf    v_hid_report_in,v__banked
;  284             hid_report_in[1] = yAxis
                               movf     v_yaxis,w,v__banked
                               movwf    v_hid_report_in+1,v__banked
;  285             hid_report_in[2] = buttons
                               movf     v_buttons,w,v__banked
                               movwf    v_hid_report_in+2,v__banked
;  288 			usb_send_data(USB_HID_ENDPOINT, hid_report_in, count( hid_report_in ) , low )
                               movlw    8
                               ...
                               bcf      v____bitbucket_15, 0,v__banked ; is_new_sequence1
                               movlw    1
